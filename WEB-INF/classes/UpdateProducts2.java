...
public class UpdateProducts2 extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{		
        ...
        HashMap<String,Product> ProductDisplays = ProductDisplay.buildBasicProductDisplayList();

        out.println("<div id='body'>");
        out.println("<section id='content'>");  
        out.println("<fieldset>"); 

        out.println("<form method='post' action='SaveProducts'>");

        for(HashMap.Entry<String, Product> myKey: ProductDisplays.entrySet()){

            String key = myKey.getKey();
           ProductDisplay ph1 = (ProductDisplay)myKey.getValue();
            ...
            if(ph1.getProductId() == productId){
                ...
                out.println("<input name='prodCategory' id='prodCategory' value='"+ph1.getType()+"'type='text'/></p>");

                out.println("<p><label for='brandName'>Brand Name:</label>");
                out.println("<input name='brandName' id='brandName' value='" + ph1.getBrandName()+"'type='text'/></p>");
